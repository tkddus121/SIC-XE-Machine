...
{	
	...
	if( strcmp(input, "loader") == 0)
	{
		error_ck = Command_Link(obj_cnt);
	}

	if( strcmp(input, "bpa") == 0)
		print_bp();
	else if( strcmp(input, "bpc") == 0)
		bp_clear();
...
char* get_commands()
{
	...
	else if( strcmp(cmd,"loader") == 0  )
	{
		...
		while(toke_str !=NULL)
		{
			sscanf(toke_str,"%s", _obj_file[obj_cnt].filename);
	...
	else if( strcmp(cmd,"bp") == 0  )
	{
		...
		while(toke_str !=NULL)
		{
			sscanf(toke_str,"%s",bp_point);

			obj_cnt++;
			toke_str = strtok(NULL," ");	
			if(obj_cnt > 0 && toke_str != NULL)
				return "ER";

			if( strcmp(bp_point , "clear")  == 0)
				return "bpc";
			else
			{
				sscanf(bp_point,"%x",&_BreakPoint_[_bp_cnt]);
				printf("\t[OK] creat breakpoint %X\n",_BreakPoint_[_bp_cnt]);
				_bp_cnt++;
			}
...
void get_opcode()
{
	...
	{
		char mnemonic[MAX_LENGTH],form[10];
		int opcode;
		opc* news = NULL;
		opc* tmps = NULL;

		sscanf(in_line,"%X %s %s",&opcode,mnemonic,form);

		news =  (opc*)malloc(sizeof(opc));
		strcpy(news->opcode,mnemonic);
		news->hash_val = opcode;
		news->next = NULL;
		strcpy(news->format,form);
